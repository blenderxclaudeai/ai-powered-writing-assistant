    ...
    const handleSignUp = async (e: React.FormEvent) => {
        ...
        if (error) {
            setError(error.message);
             ...
             // User is logged in immediately (e.g. email verification is off)
            navigate('/dashboard');
        } else if (data.user) {
            ...
            <div className="flex flex-col gap-8 bg-slate-800/60 backdrop-blur-sm p-8 rounded-xl shadow-lg border border-slate-700">
                ...
                <form onSubmit={handleSignUp} className='flex flex-col gap-4'>
                    ...
                    <div className='text-center text-xs text-slate-400'>
                        ...
                        <Link to="#" className='underline hover:text-indigo-400'>Terms of Service</Link> and{' '}
                        <Link to="#" className='underline hover:text-indigo-400'>Privacy Policy</Link>.
                    </div>
